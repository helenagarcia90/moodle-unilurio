...
class editsection_template_form extends moodleform {
    ...
    function definition() {
        ...
        // Section Name
        $mform->addElement('text', 'name', get_string('sectionname'));
        if($section->name != '') $mform->setDefault('name', $section->name);
        else $mform->setDefault('name', 'Section '.$section->section );
        $mform->setType('name', PARAM_TEXT);
        
        // Duration
        /*if($section->showavailability==0){ //if it's not an activity
            $mform->addElement('text', 'availablefrom', 'Duration en jours');
